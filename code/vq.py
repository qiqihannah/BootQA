...
def run_qpu(sample_list_total, data, sample_time, sample_size, data_name):
    ...
    for i in range(len(sample_list_total)):
        ...
        if not os.path.exists("../VQ/"+data_name+"/size_"+str(sample_size)+"/"+str(repeat)):
            os.makedirs("../VQ/"+data_name+"/size_"+str(sample_size)+"/"+str(repeat))
        df_log.to_csv("../VQ/"+data_name+"/size_"+str(sample_size)+"/"+str(repeat)+"/log.csv")

        if not os.path.exists("../VQ/" + data_name + "/size_" + str(sample_size) + "/" + str(repeat)+"/sample_info/"):
            os.mkdir("../VQ/" + data_name + "/size_" + str(sample_size) + "/" + str(repeat)+"/sample_info/")
        sampleset.to_pandas_dataframe().to_csv('../VQ/' + data_name + '/size_' + str(sample_size) + "/" + str(repeat) + "/sample_info/"+"sample_"+str(i)+".csv")
...
def print_diet(sample,data, data_name, sample_size, repeat, execution_time, max_time):
    ...
    result_df.to_csv("../VQ/"+data_name+"/size_"+str(sample_size)+"/"+str(repeat)+"/result.csv")
    sum_df.to_csv("../VQ/" + data_name + "/size_" + str(sample_size) + "/" + str(repeat) + "/sum.csv")
